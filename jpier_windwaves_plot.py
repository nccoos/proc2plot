#!/usr/bin/env python
# Last modified:  Time-stamp: <2008-10-21 16:47:14 haines>
"""jpier_windwaves_plot"""
...
ax.set_xlabel('Jpier WIND WAVES -- Last 30 days from ' + last_dt_str)
...
# ax.plot returns a list of lines, so unpack tuple
l1, = ax.plot_date(dt, Tpw, fmt='b-')
l1.set_label('Peak Wind Period (Tp)')

l2, = ax.plot_date(dt, Tmw, fmt='c-')
...
# use masked array to hide NaN's on plot
Dpw = numpy.ma.masked_where(numpy.isnan(Dpw), Dpw)
Dmw = numpy.ma.masked_where(numpy.isnan(Dmw), Dmw)

# ax.plot returns a list of lines, so unpack tuple
l1, = ax.plot_date(dt, Dpw, fmt='b-')
l1.set_label('Peak Wind Direction (Dp)')

l2, = ax.plot_date(dt, Dmw, fmt='c-')
...
# save figure
savefig('/home/haines/rayleigh/img/jpier_windwaves_last30days.png')
...
ax.xaxis.set_minor_locator( HourLocator(range(0,25,6)) )
ax.xaxis.set_major_formatter( DateFormatter('%m/%d') )
ax.set_xlabel('Jpier WIND WAVES -- Last 7 days from ' + last_dt_str)

savefig('/home/haines/rayleigh/img/jpier_windwaves_last07days.png')
...
ax.set_xticklabels([])
ax.set_xlabel('Jpier WIND WAVES -- Last 24 hours from ' + last_dt_str)
...
savefig('/home/haines/rayleigh/img/jpier_windwaves_last01days.png')
