#!/usr/bin/env python
# Last modified:  Time-stamp: <2008-10-21 16:46:27 haines>
"""jpier_swellwaves_plot"""
...
ax.set_xlabel('Jpier ADCPWAVES -- Last 30 days from ' + last_dt_str)
...
# ax.plot returns a list of lines, so unpack tuple
l1, = ax.plot_date(dt, Hss, fmt='b-')
l1.set_label('Significant Swell Wave Height (Hss)')

# l2, = ax.plot_date(dt, Hmax, fmt='g-')
...
# use masked array to hide NaN's on plot
Tps = numpy.ma.masked_where(numpy.isnan(Tps), Tps)
Tms = numpy.ma.masked_where(numpy.isnan(Tms), Tms)

# ax.plot returns a list of lines, so unpack tuple
l1, = ax.plot_date(dt, Tps, fmt='b-')
l1.set_label('Peak Swell Period (Tp)')

l2, = ax.plot_date(dt, Tms, fmt='c-')
...
# use masked array to hide NaN's on plot
Dps = numpy.ma.masked_where(numpy.isnan(Dps), Dps)
Dms = numpy.ma.masked_where(numpy.isnan(Dms), Dms)

# ax.plot returns a list of lines, so unpack tuple
l1, = ax.plot_date(dt, Dps, fmt='b-')
l1.set_label('Peak Swell Direction (Dp)')

l2, = ax.plot_date(dt, Dms, fmt='c-')
...
ax.set_xlabel('Jpier SWELL WAVES -- Last 30 days from ' + last_dt_str)
...
# save figure
savefig('/home/haines/rayleigh/img/jpier_swellwaves_last30days.png')
...
ax.set_xticklabels([])
ax.set_xlabel('Jpier ADCPWAVES -- Last 7 days from ' + last_dt_str)
...
savefig('/home/haines/rayleigh/img/jpier_swellwaves_last07days.png')
...
ax.set_xticklabels([])
ax.set_xlabel('Jpier ADCPWAVES -- Last 24 hours from ' + last_dt_str)
...
savefig('/home/haines/rayleigh/img/jpier_swellwaves_last01days.png')
